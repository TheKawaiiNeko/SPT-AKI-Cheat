...
void Tarkov::UpdateGlobalData()
{
    ...
    {   
        ...
        GameWorld = GetObjectFromList(activeNodes, lastActiveNode, "GameWorld"); // OK

        if (GameWorld == 0)
            printf("Invalid GameWorld!\n");

        // LocalGameWorld
        uint64_t LGWt1 = m.Read<uint64_t>(GameWorld + 0x30);
        uint64_t LGWt2 = m.Read<uint64_t>(LGWt1 + 0x18);
        uint64_t NewLocalGameWorld = m.Read<uint64_t>(LGWt2 + 0x28);

        if (NewLocalGameWorld != 0 && LocalGameWorld != NewLocalGameWorld)
            LocalGameWorld = NewLocalGameWorld;
